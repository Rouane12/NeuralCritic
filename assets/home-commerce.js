(() => {
  ...
  async function init(){
    ...
    const [productsRes,retailersRes]=await Promise.all([
      ...
      supa.from('commerce_retailers').select('id,slug,name,affiliate_network,active').in('id',retailerIds).eq('active',true)
    ]);
    ...
    const amazonPicks=picks.filter(offer=>retailers.get(offer.retailer_id)?.affiliate_network==='amazon-associates');
    const amazonFreshest=amazonPicks.reduce((latest,offer)=>!latest||new Date(offer.fetched_at)>new Date(latest)?offer.fetched_at:latest,null);
    const amazonNotice=amazonPicks.length
    ...
    }).join('')}</div><p class="home-commerce-note">Some outbound links may be affiliate links. Neural Critic may earn a commission from qualifying purchases.${amazonNotice} <a href="commercial.html">Commercial disclosure</a>.</p></div>`;
    host.hidden=false;
