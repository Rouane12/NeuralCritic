(() => {
  ...
  async function syncAuthorFollow(){const name=authorName(),buttons=$$('[data-author-follow],[data-article-follow]');buttons.forEach(b=>b.classList.remove('active','community-active'));if(!session?.user||!buttons.length)return;const {data}=await client.from('author_follows').select('author_name').eq('user_id',session.user.id).eq('author_name',name).maybeSingle();buttons.forEach(button=>{button.classList.toggle('community-active',!!data);if(button.matches('[data-author-follow]'))button.textContent=data?'FOLLOWING':'FOLLOW';const small=$('small',button);if(small)small.textContent=data?'Following':'Follow'})}
