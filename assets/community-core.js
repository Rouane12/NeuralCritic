(() => {
  ...
  async function refreshCommunity(){updateAccountButton();bindAuthorFollow();await syncAuthorFollow();window.dispatchEvent(new CustomEvent('neuralcritic:community-refreshed'))}
  async function init(){
    ...
    await waitFor('.header-tools');await refreshCommunity();
