...
function matchCategory(a, filter){
  ...
  return (a.tags||[]).some(t=>t.toLowerCase()===f);
}
...
function renderHome(){
  ...
  document.getElementById('newsletter-form')?.addEventListener('submit',e=>{e.preventDefault(); e.currentTarget.innerHTML='<p class="notice">You’re on the list. Welcome to the Weekly Drop.</p>';});
}
...
function renderCategory(){
  ...
  document.getElementById('category-title').textContent=cat==='latest'?'Latest stories':cat.charAt(0).toUpperCase()+cat.slice(1);
...
async function init(){
  ...
  wireChrome();
  try{ ARTICLES=await fetch(DATA_URL).then(r=>r.json()); }catch(e){ console.error(e); return; }
  renderHome(); renderArticle(); renderCategory(); renderSearchPage();
}
init();
