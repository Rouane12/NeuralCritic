(() => {
  ...
  const GENERIC_TAGS = new Set(['review','reviews','feature','features','news','guide','guides','pc','playstation','xbox','nintendo','mobile','action rpg','action-rpg','rpg','open world','open-world','gaming','game','games']);
  ...
  function slugify(value = '') { return String(value).trim().toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, ''); }
  ...
  function normalizedTags(article) { return (Array.isArray(article?.tags) ? article.tags : []).map(tag => String(tag || '').trim().toLowerCase()).filter(Boolean); }
  function meaningfulTags(article) { return normalizedTags(article).filter(tag => !GENERIC_TAGS.has(tag)); }
  function titleTokens(article) { return new Set(String(article?.title || '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').split(/\s+/).filter(token => token.length > 2 && !STOP_WORDS.has(token))); }
  ...
  function identityValue(article, key) {
    const aliases = { game: ['gameKey','game_key'], series: ['series'], franchise: ['franchise'] };
  ...
  function sameIdentity(a, b) { return !!a && !!b && slugify(a) === slugify(b); }
  ...
  function relationFor(current, candidate) {
    ...
    if (sameIdentity(currentGame, identityValue(candidate, 'game'))) return { key:'same_game', weight:60, type:'game', value:currentGame, label:`More ${displayTag(currentGame)}` };
    const currentSeries = identityValue(current, 'series');
    if (sameIdentity(currentSeries, identityValue(candidate, 'series'))) return { key:'same_series', weight:42, type:'series', value:currentSeries, label:`More ${displayTag(currentSeries)}` };
    const currentFranchise = identityValue(current, 'franchise');
    if (sameIdentity(currentFranchise, identityValue(candidate, 'franchise'))) return { key:'same_franchise', weight:24, type:'franchise', value:currentFranchise, label:`More ${displayTag(currentFranchise)}` };
  ...
  function scoreCandidate(current, candidate) {
    ...
    const meaningfulOverlap = meaningfulTags(current).filter(tag => meaningfulTags(candidate).includes(tag));
    const allOverlap = currentTags.filter(tag => candidateTags.includes(tag));
    const candidateTokens = titleTokens(candidate);
    const titleOverlap = [...titleTokens(current)].filter(token => candidateTokens.has(token)).length;
    let score = relation.weight + meaningfulOverlap.length * 14 + Math.min(6, allOverlap.length * 2);
    ...
    score += Math.min(8, titleOverlap * 3) + freshness(candidate);
  ...
  function reasonFor(current, candidate, scored) {
    ...
    if (scored.meaningfulOverlap.length) {
      ...
      return { key:'topic', type:'', value:tag, label:/^fromsoftware$/i.test(tag) ? `More from ${displayTag(tag)}` : `More ${displayTag(tag)}` };
    }
    if (String(current.category || '').toLowerCase() === String(candidate.category || '').toLowerCase()) return { key:'category', type:'', value:'', label:`More ${String(current.category || 'stories').toLowerCase()}` };
  ...
  async function loadPublished() {
    ...
    try { const response = await fetch('data/articles.json'); return response.ok ? await response.json() : []; }
    catch (_) { return []; }
  }
  async function loadCurrent(slug, index) {
    ...
    try { const response = await fetch(`data/articles/${encodeURIComponent(slug)}.json`); if (response.ok) return await response.json(); }
    catch (_) {}
  ...
  function cardMarkup(item, position) {
    ...
    return `<a class="${classes}" href="${storyUrl(article.slug)}" data-recirc-target="${esc(article.slug)}" data-recirc-reason="${esc(item.reason.key)}"><div class="nc-recirc-media">${image ? `<img src="${esc(image)}" alt="${esc(article.imageAlt || article.title || '')}" loading="lazy" decoding="async">` : '<span>NEURAL<br>CRITIC</span>'}<i>${position === 0 ? 'NEXT READ' : esc(item.reason.label.toUpperCase())}</i></div><div class="nc-recirc-copy"><div class="nc-recirc-meta"><span>${esc(category)}</span>${minutes ? `<b>${minutes} MIN READ</b>` : ''}</div><h3>${esc(article.title || '')}</h3>${position === 0 ? `<p>${esc(article.description || '')}</p>` : ''}<strong>${position === 0 ? 'KEEP READING' : 'READ STORY'} <span aria-hidden="true">→</span></strong></div></a>`;
  }
  ...
  function trackClicks(module) {
    module.addEventListener('click', event => {
      ...
      if (link) window.NeuralCriticAnalytics?.track?.('recirculation_click', { placement:'after_thread', target_slug:link.dataset.recircTarget || '', recommendation_reason:link.dataset.recircReason || '' });
      const hub = event.target.closest('[data-recirc-hub]');
      if (hub) window.NeuralCriticAnalytics?.track?.('recirculation_hub_click', { placement:'after_thread', hub_type:hub.dataset.recircHub || '', hub_value:hub.dataset.recircHubValue || '' });
    });
    if (!('IntersectionObserver' in window)) return;
    const observer = new IntersectionObserver(entries => {
      if (!entries.some(entry => entry.isIntersecting)) return;
      observer.disconnect();
      window.NeuralCriticAnalytics?.track?.('recirculation_view', { placement:'after_thread', recommendation_count:module.querySelectorAll('[data-recirc-target]').length, primary_reason:module.dataset.primaryReason || '' });
    }, { threshold: 0.25 });
    observer.observe(module);
  }
  function bestHub(current, primary) {
    if (primary?.relation?.type && primary.relation.value) return { href:topicUrl(primary.relation.type, primary.relation.value), type:primary.relation.type, value:primary.relation.value };
  ...
  async function init() {
    ...
    if (!current) return;
    const ranked = index.filter(article => article?.slug && article.slug !== slug).map(article => { const scored = scoreCandidate(current, article); return { article, ...scored, reason:reasonFor(current, article, scored) }; }).sort((a,b) => b.score - a.score || new Date(b.article.publishedAt || 0) - new Date(a.article.publishedAt || 0));
    const selected = [];
    const seen = new Set();
    const primary = ranked.find(item => item.relation?.key) || ranked.find(item => item.meaningfulOverlap.length) || ranked[0];
    if (primary) { selected.push(primary); seen.add(primary.article.slug); }
    ranked.forEach(item => { if (selected.length < 3 && !seen.has(item.article.slug)) { selected.push(item); seen.add(item.article.slug); } });
    ...
    const exploreLabel = hub.href && primaryIdentity ? `EXPLORE ${displayTag(primaryIdentity).toUpperCase()} →` : 'EXPLORE MORE →';
    ...
    trackClicks(module);
    window.dispatchEvent(new CustomEvent('neuralcritic:recirculation-ready', { detail:{ sourceSlug:slug, primarySlug:selected[0]?.article?.slug || '', primaryReason:selected[0]?.reason?.key || '', primaryScore:selected[0]?.score || 0 } }));
  }
